...
for index in range(0,3):
    ...
    for k in range(1,6):
        ...
        data = np.loadtxt(pathDict[index] + 'one-' + str(k) + '-mer.csv', dtype=np.float64, delimiter=',')
        np.random.shuffle(data)
        vector_size = (4**k) * (SEQUENCE_SIZE-k+1)
        x = data[:,:vector_size]
        print(*x)
    ...
    print('Imprimindo estatísticas para representação oneHot de k-mers.')
    # printar a lista em ordem
    resultFile = open(pathDict[index] + 'one-results.txt','w')
