...
for index in range(0,3):
    ...
    for k in range(1,8,1):
        ...
        data = np.loadtxt(pathDict[index] + 'word-' + str(k) + '-mer-'+ str(WINDOW_SIZE) +'-win-' + str(VECTOR_SIZE) + '-size.csv', dtype=np.float64, delimiter=',')
        np.random.shuffle(data)
        vector_size = VECTOR_SIZE * (SEQUENCE_SIZE-k+1)
        x = data[:,:vector_size]
        print(*x)
    ...
    print('Imprimindo estatísticas para representação em Word2Vec de k-mers.')
    # printar a lista em ordem
    resultFile = open(pathDict[index] + 'word-' + str(WINDOW_SIZE) + '-win-' + str(VECTOR_SIZE) + '-siz-results.txt','w')
