...
for k in range(1,8):
    ...
    for index in range(0,3):
        ...
        model = FastText.load(pathDict[index]+"model-fast-"+str(k)+"-mer-"+str(windowSize)+"-win-"+str(vectorSize)+"-size.model")

        # abre o arquivo no qual escreve as sequências de vetores
        vectorFile = open(pathDict[index]+"fast-"+str(k)+"-mer-"+str(windowSize)+"-win-"+str(vectorSize)+"-size.csv",'w')
